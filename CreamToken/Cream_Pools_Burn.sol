...
contract BurnPool is Ownable {
    ...
    /**
     * @dev Deposit CREAM tokens to Faucet for rewards allocation and/or withdraw outstanding rewards.
     * @param _amount Amount of CREAM tokens to deposit.
     */
    function transact(uint256 _amount) public {
        ...
        if (block.number < lockBlock && _amount != 0) {
            ...
            stakedCream = stakedCream.add(_amount);
        ...
        if (block.number >= endBlock) {
            CREAM.safeTransfer(burnAddress, user.amount);
