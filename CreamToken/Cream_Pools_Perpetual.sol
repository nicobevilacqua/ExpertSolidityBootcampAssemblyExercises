...
contract PerpetualPool is Ownable {
    ...
    IERC20 public immutable CREAM; // Cream token.
    ...
    uint256 public stakedCream; // Total Cream tokens staked in the contract.
    PoolInfo public poolInfo;
    address public feeTo;

    mapping(address => UserInfo) public userInfo; // Info of each user that stakes CREAM tokens.
    ...
    constructor(
        ...
        require(address(_CREAM) != address(0), "_CREAM address not set!");
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > poolInfo.lastRewardBlock && stakedCream != 0) {
            ...
            accRewardPerShare = accRewardPerShare.add(
                tokenReward.mul(1e12).div(stakedCream)
            );
        }
        return
    ...
    /**
     * @dev Deposit Cream tokens to the Perpetual Pool for reward allocation.
    ...
    function deposit(uint256 _amount) public {
        ...
        CREAM.safeTransferFrom(address(msg.sender), feeTo, fee);
        CREAM.safeTransferFrom(
            address(msg.sender),
            address(this),
            _amount - fee
        );
        stakedCream = stakedCream.add(_amount - fee);
    ...
    /**
     * @dev Withdraw Cream tokens from the Perpetual Pool.
    ...
    function withdraw(uint256 _amount) public {
        ...
        require(
            user.amount >= _amount,
            "Can't withdraw more tokens than previously deposited."
        );
        updatePool();
        uint256 pending = user
            .amount
        ...
        stakedCream = stakedCream.sub(_amount);
    ...
    function emergencyWithdraw() public {
        ...
        stakedCream = stakedCream.sub(user.amount);
    ...
    function emergencyTransfer(address _to) external onlyOwner returns (bool) {
        if (CREAM == REWARD) {
            return
                REWARD.transfer(
                    _to,
                    REWARD.balanceOf(address(this)) - stakedCream
                );
        } else {
     ...
     */
    function setCreamPerBlock(uint256 _rewardPerBLock)
        external
        onlyOwner
        returns (uint256)
    {
        require(_rewardPerBLock > 0, "Cream per Block must be greater than 0.");
        updatePool();
        rewardPerBlock = _rewardPerBLock;
